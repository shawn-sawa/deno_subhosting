...
export default class Client {
  ...
  constructor(accessToken?: string, orgId?: string, options?: ClientOptions) {
    ...
    if (!at) {
      throw new Error("A Deno Deploy access token is required (or set DEPLOY_ACCESS_TOKEN env variable).");
    }

    const org = orgId ?? Deno.env.get("DEPLOY_ORG_ID");
    if (!org) {
      throw new Error("Deno Subhosting org ID is required (or set DEPLOY_ORG_ID env variable).");
    }
